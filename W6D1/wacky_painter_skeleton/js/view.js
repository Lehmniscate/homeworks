...
View.prototype.exercise4 = function () {
  ...
  //your code here!
  let $squares = $('li');
  $squares.each((index, s) => {if(index % 2 !== 0) $(s).text('Michael');});
};
...
View.prototype.exercise5 = function () {
  ...
  //your code here!
  $('ul').each(function(i, row_ul) {
    let row = i;
    $(row_ul).children().each(function(j, li) {
      let col = j;
      $(li).on("click", () => alert(`${row}, ${col}`));
    });
  });
};
...
View.prototype.exercise7 = function(){
  ...
  $('li').on(
    "mouseover",
    (i) => console.log($(i.currentTarget).css("background-color"))
  );
};
